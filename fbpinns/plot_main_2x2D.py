...
def _plot_quiver(x_test, yj, bsize):
    ...
    if yj.shape[1] == 3:
        ...
        plt.contourf(x[:,:,0], x[:,:,1], y[:,:,2], alpha=0.5, cmap=cm.viridis)
        plt.colorbar()
        plt.contour(x[:,:,0], x[:,:,1], y[:,:,2], cmap=cm.viridis)
...
def plot_2x2D_PINN(x_test, yj_true,   x, yj,   yj_full, y_full_raw,   yj_test_losses,   c, i):
    ...
    _plot_quiver(x_test, y_full_raw, c.BATCH_SIZE_TEST)
    ...
    _plot_quiver(x_test, boundary[0], c.BATCH_SIZE_TEST)
    #_plot_test_im(boundary[0][:,0], xlim, (None, None), c)
    #_fix_plot(xlim)
    plt.title("[%i] Boundary condition"%(i,))
