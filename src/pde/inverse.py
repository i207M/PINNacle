...
class PoissonInv(baseclass.BasePDE):
    ...
    def __init__(self):
        ...
        def pde(x, ua):
            ...
            d_au = dde.grad.jacobian(a * u_x, x, i=0, j=0) + \
                   dde.grad.jacobian(a * u_y, x, i=0, j=1)

            def f(xy):
                x, y = xy[:, 0:1], xy[:, 1:2]
                return 2 * np.pi**2 * torch.sin(np.pi * x) * torch.sin(np.pi * y) * a_ref(xy) + \
                    2 * np.pi * ((2*x+1) * torch.cos(np.pi * x) * torch.sin(np.pi * y) + (2*y+1) * torch.sin(np.pi * x) * torch.cos(np.pi * y)) * a_ref(xy)**2
        ...
        self.ref_sol = (lambda xy: np.concatenate((u_ref(xy), a_ref(xy)), axis=1))

        self.recommend_net = dde.nn.PFNN([self.input_dim] + 5 * [[50] * self.output_dim] + [self.output_dim], "tanh", "Glorot normal")
        ...
        self.add_bcs([{
            'component': 0,
            'points': bc_xy,
            'values': u_ref(bc_xy) + np.random.normal(loc=0, scale=0.1, size=(2500, 1)),
            'type': 'pointset',
            'name': 'data_loss',
        }, {
            'component': 1,
...
class HeatInv(baseclass.BaseTimePDE):
    ...
    def __init__(self):
        ...
        self.recommend_net = PFNN([self.input_dim] + 5 * [[50] * self.output_dim] + [self.output_dim],
                                  "tanh",
                                  "Glorot normal",
