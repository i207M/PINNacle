...
class Model:
    ...
    def _compile_tensorflow_compat_v1(self, lr, loss_fn, decay, loss_weights):
        ...
        def losses(losses_fn):
            # Data losses
            losses = losses_fn(self.net.targets, self.net.outputs, loss_fn, self.net.inputs, self)
        ...
        self.train_step = optimizers.get(total_loss, self.opt_name, learning_rate=lr, decay=decay)
    ...
    def _compile_tensorflow(self, lr, loss_fn, decay, loss_weights):
        ...
        def train_step(inputs, targets, auxiliary_vars):
            ...
            trainable_variables = (self.net.trainable_variables + self.external_trainable_variables)
            grads = tape.gradient(total_loss, trainable_variables)
            opt.apply_gradients(zip(grads, trainable_variables))

        def train_step_tfp(inputs, targets, auxiliary_vars, previous_optimizer_results=None):
        ...
        self.train_step = (train_step if not optimizers.is_external_optimizer(self.opt_name) else train_step_tfp)
    ...
    def _compile_paddle(self, lr, loss_fn, decay, loss_weights):
        ...
        trainable_variables = (list(self.net.parameters()) + self.external_trainable_variables)
        self.opt = optimizers.get(trainable_variables, self.opt_name, learning_rate=lr, decay=decay)
        ...
        self.outputs_losses_train = outputs_losses_train
        self.outputs_losses_test = outputs_losses_test
        self.train_step = (train_step if not optimizers.is_external_optimizer(self.opt_name) else train_step_lbfgs)
    ...
    def state_dict(self):
        ...
        if backend_name == "tensorflow.compat.v1":
            ...
            destination = {f"external_trainable_variable:{i}": v for (i, v) in enumerate(self.external_trainable_variables)}
            # the paramaters of the net
            destination.update(self.net.get_weight_paths())
        elif backend_name in ["pytorch", "paddle"]:
            destination = self.net.state_dict()
        else:
            raise NotImplementedError("state_dict hasn't been implemented for this backend.")
        return destination
    ...
    def save(self, save_path, protocol="backend", verbose=0):
        ...
        if protocol == "pickle":
            ...
            if backend_name == "tensorflow.compat.v1":
                ...
                raise NotImplementedError("Model.save() hasn't been implemented for this backend.")
        if verbose > 0:
    ...
    def restore(self, save_path, verbose=0):
        ...
        if backend_name == "tensorflow.compat.v1":
            ...
            raise NotImplementedError("Model.restore() hasn't been implemented for this backend.")
