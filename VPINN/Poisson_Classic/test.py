...
def bc(boundary_num):
    ...
    us.extend([torch.ones_like(x_r), torch.ones_like(x_u), torch.ones_like(x_l), torch.ones_like(x_d)])
...
def pde(x, y, u):    
    return VPINN.gradients(u, x, 2) + VPINN.gradients(u, y, 2)

#############################################################################################
# train the model
def poisson_classic(epoch=10000):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    vpinn = VPINN([2, 20, 20, 20, 1],pde, type='tanh', bc1=bc(80), area=[-0.5, 0.5, -0.5, 0.5], Q=10, grid_num=8, test_fcn_num=5, 
                device=device, load=None)


    net = vpinn.train("Poisson_Classic", epoch_num=epoch, coef=10 ** (3 - np.log2(vpinn.grid_num)))
    net.cpu()
    # verify
    data = np.loadtxt('poisson_classic.dat', skiprows=0)
    ...
    # # image 1
    # image1 = axes[0].imshow(prediction.detach().numpy(), cmap='jet', origin='lower', extent=[-0.5, 0.5, -0.5, 0.5])
